...
**Remote functions are the primary data layer**, not `load()` functions — see the "Data fetching" section of `README.md` for the full rule, examples, and the list of cases where `load()` is still correct. In short: `src/lib/remotes/*.remote.ts` (`bills`, `households`, `payments`, `dashboard`, `common`, `images`) expose `query(...)` for reads and `form(...)` for mutations; components `await` those inside a `<svelte:boundary>` rather than receiving data from a parent `load`; mutations invalidate via `.refresh()` on the related query, never `invalidate()` / `invalidateAll()`. `load()` stays only for the root session layer (`+layout.server.ts`) and for route-level redirects/guards (`login`, `register`, `signup`, `logout`, the `payments` → `bills` redirect).
