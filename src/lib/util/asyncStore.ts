import { writable, readable, type Readable, type Writable, type StartStopNotifier } from 'svelte/store';
 ...
 */
export function asyncReadableStore<R>(initial: R, fn: (set: Parameters<StartStopNotifier<R>>[0]) => Promise<R>): Readable<R> {
 ...
 * @returns 
 */
export function asyncWritableStore<R>(initial: R, fn: (set: Parameters<StartStopNotifier<R>>[0]) => Promise<R>): Writable<R> {
...
export function asyncStore() {
  ...
  return {
    subscribe: (fn: (v: unknown) => void | Promise<void>) => subscribers.add(fn),
  };
}
