...
<Drawer open={open} on:close let:close={closeMe}>
  <form action="/dashboard/bills?/addBill" method="post" class="p-4" use:enhance={submit}>
    ...
    <section class="grid grid-cols-3 gap-3">
      <div>
        <FormLabel label="Name">
          <input class="input" name="name" placeholder="Name of the bill" required />
        </FormLabel>
      </div>
      <div>
        <FormLabel label="Household">
          <select class="select" name="household-id" required placeholder="Please select a household">
      ...
      <div>
        <FormLabel label="Due date">
          <input class="input" name="due-date" type="number" min="1" max="28" required placeholder="Due date between 1 and 28">
        </FormLabel>
      </div>
      <div class="col-span-3 flex justify-end gap-3">
        <Button variant="filled" on:click={() => closeMe()}>
          Close
        </Button>
        <Button type="submit">
          Add
